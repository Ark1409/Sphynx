...
namespace Sphynx.Core
{
    ...
    // and while most performed about the same or even better in a select few cases, this one
    // was ultimately chosen given that it has proven to be the most optimal for high-contention
    // scenarios, while still offering similar performance when contention is low.
    //
        ...
        public static SnowflakeId NewId()
        {
            while (true)
            {
                lock (_idLock)
                {
                    ...
                    return new SnowflakeId(timestamp, _lastSequence, MACHINE_ID);
                }
            }
        }
