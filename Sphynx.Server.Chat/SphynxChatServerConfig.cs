...
namespace Sphynx.Server.Chat
{
    public record SphynxChatServerConfig
    {
        public const string FILENAME = ".env";

        public static SphynxChatServerConfig Default => new()
        {
            ...
            RateLimiterPermits = 25,
            RateLimiterPeriod = TimeSpan.FromSeconds(1),
        };
        ...
        [PropertiesMember("redis.connection_string")]
        ...
        public static SphynxChatServerConfig LoadFromEnvironment()
        {
            var config = Default;

            string? portEnv = Environment.GetEnvironmentVariable("SPHYNX_PORT");
            ...
            string? redisConnStringEnv = Environment.GetEnvironmentVariable("SPHYNX_REDIS_CONN");
            ...
            string? rlPermitsEnv = Environment.GetEnvironmentVariable("SPHYNX_RL_PERMITS");
            string? rlPeriodEnv = Environment.GetEnvironmentVariable("SPHYNX_RL_PERIOD");
            ...
            if (!string.IsNullOrWhiteSpace(redisConnStringEnv)) config.RedisConnectionString = redisConnStringEnv.Trim();
